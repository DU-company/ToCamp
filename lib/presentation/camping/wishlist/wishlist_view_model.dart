...
class WishlistViewModel extends Notifier<List<WishlistModel>> {
  ...
  Future<void> getWishlist() async {
    final resp = await repository.getAllWishlist();
    state = resp;
  }
  ...
  void onLikePressed({
    ...
    if (isLiked) {
      /// 이미 좋아요가 눌러져있으면 삭제
      repository.removeFromWishlist(campingModel.id);
      // 새로 데이터를 받아온다.
      getWishlist();
    } else {
      /// 카테고리 선택
  ...
  Future<void> onCardTap({
    ...
    if (isAdding) {
      context.pop();
      await repository.addToWishlist(wishlistModel.id, campingModel!);
      getWishlist();
  ...
  Future<void> onCreateWishlist(
    BuildContext context,
    CampingModel campingModel,
  ) async {
    try {
      final name = ref.read(wishlistNameProvider);
      _validateName(name);
  ...
  Future<void> deleteCategory(WishlistModel model) async {
    await repository.deleteWishlist(model.id);
    state = state.where((e) => e.id != model.id).toList();
    ref
        .read(toastServiceProvider)
        .showToast(text: '"${model.name}"위시리스트가 삭제되었습니다');
  }
