...
import 'package:to_camp/presentation/common/widgets/app_info.dart';
import 'package:to_camp/presentation/banner_ad/banner_ad_view.dart';
import 'package:to_camp/presentation/home/widgets/home_app_bar.dart';
import 'package:to_camp/presentation/home/widgets/mini_card/mini_card_list_view.dart';
...
class _CategoryMiniList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(searchViewModelProvider(keyword));
    final viewModel = ref.read(
      searchViewModelProvider(keyword).notifier,
    );
    return MiniCardListView(
      label1: label,
      label2: '더 보기',
      state: state,
      onReadMore: () => viewModel.routeToCampingScreen(context),
...
class BasedMiniList extends ConsumerWidget {
  const BasedMiniList({super.key});
...
class RecentMiniList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final pState = PaginationSuccess(items: state, hasMore: true);

    return MiniCardListView(
      label1: '최근 본 캠핑장',
      label2: '더 보기',
      state: pState,
      onReadMore: () => viewModel.onRecentCampingTap(context),
      onRefresh: () => viewModel.paginate(),
